...
// AAD object IDs of users / groups / SPs that will create or update hosted agents.
// They get Managed Identity Operator on the account + Network Contributor on the agent subnet.
// Leave empty to skip.
...
// Leave empty to auto-name; the project will get an `appinsights` connection
// and the project MI gets Log Analytics Reader + Azure AI User on the account.
