...
| **"responses protocol not declared"** | agent.yaml only declares `invocations` but bot/CLI calls via Responses API | Declare BOTH protocols in agent.yaml: `invocations` (for container runtime) + `responses` (for bot, azd invoke, external callers) |
