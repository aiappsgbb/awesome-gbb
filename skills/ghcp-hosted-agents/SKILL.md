...
| **"responses protocol not declared"** | agent.yaml only declares `invocations` but bot/CLI calls via Responses API | **Option A:** Declare both protocols in agent.yaml (invocations + responses) — UNTESTED, may not work if container only serves invocations. **Option B:** Switch bot to call via Invocations SSE endpoint instead of `oai.responses.create()`. **Option C:** Use MAF runtime (ResponsesHostServer) which natively serves responses. |
