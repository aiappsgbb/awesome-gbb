...
`docker.imagePassthrough: true` / `remoteBuild: false`; never a digest through
`--from-package`. A matching connection is reused; missing configuration routes
...
The new consumer's live acceptance is pending until its exact candidate
activates and completes a native model request with independent response/session
readback. Historical private BASIC evidence does not certify these new files.
...
- **Add** explicit `mcp~=1.29.0` — **mandatory**, not conditional. `agent_framework_foundry_hosting._responses`
