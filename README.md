...
| [**foundry-webiq**](skills/foundry-webiq/) | Ground a hosted agent on **Microsoft Web IQ** — AI-native web/news/image/video grounding (MCP-native JSON-RPC 2.0 + REST, citation-ready JSON). Covers the MCP tool connection (**API-key static header + Microsoft Entra bearer**), the consumer `MCPStreamableHTTPTool` pattern with **runtime tool auto-discovery** (no hardcoded JSON-RPC method names), a REST fallback, and provenance/citation mapping. Endpoint, key header, and Entra scope are **consumer-supplied from your gated Web IQ docs** — never invented. Limited-access (`aka.ms/webiq-waitlist`) |
