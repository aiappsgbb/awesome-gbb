...
| [**progress-guard**](skills/progress-guard/) | **Draft source candidate:** execution memory, bounded recovery and compaction-safe handoffs. Reuses existing plans/todos; preserves evidence and failed attempts with selective snapshot readback. No supervisors, runtime threshold control or tool preemption. [Behavioral acceptance pending](docs/maintenance/progress-guard-validation.md). |
