...
| [**foundry-vnet-deploy**](skills/foundry-vnet-deploy/) | Deploy Foundry with **Agent Setup inside a private VNet** — preserves the vendored templates, fixed-timestamp retry and optional Citadel peering/APIM DNS modules. Adds ownership-aware intake and read-only account/PE/subnet inventory, keeping management-plane observations separate from private runtime connectivity and gating recovery writes explicitly. |
| [**foundry-toolbox**](skills/foundry-toolbox/) | Use Foundry Toolbox GA with stable `AIProjectClient.toolboxes` CRUD, authenticated hosted-agent wiring, immutable version promotion/rollback and stable Tool Search. Includes direct/nested consent error handling, Streamable HTTP approval discovery, exact proxy tool names, and the `azure_ai_search`-is-INDEX-not-KB boundary without restoring retired preview headers. |
| [**foundry-skill-catalog**](skills/foundry-skill-catalog/) | Foundry Skills preview API with native immutable versions, default promotion/rollback and pinned downloads. Preserves the **`FoundrySkillsSource(SkillsSource)`** entry point and adds version-aware build-time bundles; distinguishes current inline/ZIP content from legacy `has_blob` compatibility and Toolbox MCP Resources discovery from ordinary tool discovery. |
...
| [**progress-guard**](skills/progress-guard/) | **Follow-up candidate:** execution memory, bounded recovery and quiet child coordination. Explicit assignment/skill loading, terminal handoffs, parent verification and selective snapshot readback. No supervisors, runtime threshold control or tool preemption. [Behavioral acceptance pending](docs/maintenance/progress-guard-validation.md). |
...
> [!TIP]
> **Install the catalog in one command via plugin.** The proposed catalog contains 41 skills in a single [Copilot CLI plugin](plugin.json). Main includes AgentOps, delegated-auth and web-experience-design source; progress-guard is a source candidate. Installation does not certify their remaining acceptance gates:
>
> ```bash
