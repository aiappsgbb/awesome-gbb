...
| [**foundry-toolbox**](skills/foundry-toolbox/) | Manage GA Toolbox versions, A2A 1.0 and BM25 Tool Search with explicit pinning/search metadata. Preserves hosted runtime pins, consent/approval boundaries and exact discovered names; labels the Prompt bridge and Skills separately as preview and records A2A service-managed retention separately from active-resource cleanup. |
| [**foundry-skill-catalog**](skills/foundry-skill-catalog/) | Versioned Skills preview API with pinned/default downloads, build-time bundles and the compatible **`FoundrySkillsSource(SkillsSource)`** adapter. Includes the existing MAF 1.17 native MCP provider for progressive inline-body loading, distinct from eager download/injection, with explicit approval and version boundaries. |
