...
def build(out_dir: pathlib.Path, *, validate: bool) -> int:
    ...
    # Clean output directory (preserving .gitkeep)
    if out_dir.exists():
    ...
    # Backward-compat redirect for the legacy /threadlight-experience.html URL.
    # Pre-flip, Pages served the file at the repo root. Post-flip, Pages serves
    # docs/, so we need this stub for old bookmarks/links to land at the new
    # canonical /threadlight/ URL.
    redirect_html = (
        '<!doctype html>\n'
        '<html lang="en">\n'
        '<head>\n'
        ...
        '<meta http-equiv="refresh" content="0; url=/threadlight/">\n'
        '<link rel="canonical" href="/threadlight/">\n'
        '</head>\n'
        '<body>\n'
        '<p>The Threadlight experience has moved to '
        '<a href="/threadlight/">/threadlight/</a>.</p>\n'
        '<script>location.replace("/threadlight/");</script>\n'
        '</body>\n'
        '</html>\n'
    )
    total_bytes += _write(out_dir / 'threadlight-experience.html', redirect_html)
    html_count += 1
